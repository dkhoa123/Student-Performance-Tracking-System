...
namespace SPTS_Repository.Repositories.Giangvien
{
    public class SectionTeacherRepository : ISectionTeacherRepository
    {
        ...
        public async Task<List<SectionCardViewModelDto>> GetSectionsByTeacherAsync(int teacherId, int? termId = null)
        {
            // Bước 1: Query data với filter theo term (nếu có)
            ...
            // ✅ Thêm filter theo termId nếu có
            if (termId.HasValue)
            {
                query = query.Where(sec => sec.TermId == termId.Value);
            }

            var sections = await query.ToListAsync();

            // Bước 2: Map sang DTO trong memory (giữ nguyên phần này)
